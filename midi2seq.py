...
#* Helper function to process_midi_seq
#* Very important, works as a mask to extract a part of the sequence
...
#* This function may be modified to be a batch processor
def process_midi_seq(all_midis=None, datadir='data', n=10000, maxlen=50):
    """
    Notes by AZ
    * Each midi file contains about 10 ~ 20 min worth of piano music
    * When called with piano2seq comes out around 40k - 50k int notes in a 1D array
    * The segment() and np.vstack reshuffles them into a 2D matrix where each row has maxlen number of notes in sequence
    """
...
#* Function that creates random piano notes and creates chains them together to a midi file
